...
class InteriorGenerator:
    ...
    def generate(self, description: str) -> dict:
        ...
        根据描述生成建筑内部。
        Args:
            description: 内部的自然语言描述，例如 "一个带床和工作台的卧室"。
        Returns:
            一个字典，包含内部的方块布局和结构信息。
        """
        print(f"InteriorGenerator: Generating interior for description: {description}")

        system_prompt = """你是一个Minecraft室内设计师。你的任务是根据用户的描述，设计一个Minecraft建筑内部的布局和家具，并输出一个JSON数组，其中每个元素都是一个几何图元生成任务。每个任务都必须包含'tool'和'args'字段。

可用的几何图元工具及其参数示例：
- 'cube': {'x': 0, 'y': 0, 'z': 0, 'size_x': 1, 'size_y': 1, 'size_z': 1, 'block_type': 'minecraft:crafting_table'} (用于家具)
- 'single_block': {'x': 0, 'y': 0, 'z': 0, 'block_type': 'minecraft:bed'}
- 'line': {'x1': 0, 'y1': 0, 'z1': 0, 'x2': 5, 'y2': 0, 'z2': 0, 'block_type': 'minecraft:bookshelf'} (用于书架)

重要提示:
1.  所有坐标都是相对坐标，以(0,0,0)为基准。
2.  'block_type'必须是有效的Minecraft方块ID，例如'minecraft:bed'、'minecraft:crafting_table'、'minecraft:furnace'。
3.  请严格按照JSON数组格式返回，不要包含任何额外说明或代码块标记。"""    
        user_prompt = f"设计一个Minecraft建筑内部：{description}"
        print(f"InteriorGenerator: Sending prompt to LLM: {user_prompt}")

        success, llm_output = get_llm_response(self.llm_client, system_prompt, user_prompt)

        if not success:
            print(f"InteriorGenerator: Error from LLM: {llm_output}")
            return {"description": description, "generated_structure": {}, "blocks": []}
        
        generated_components = llm_output
        print(f"InteriorGenerator: Received LLM output: {generated_components}")
        ...
        print(f"InteriorGenerator: Finished generating plan for: {description}")
        return build_plan

if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='Interior Generator for Minecraft AI Builder.')
    ...
    generator = InteriorGenerator(llm_client)
    ...
    print(f"InteriorGenerator: Saved build plan to {output_filepath}")
