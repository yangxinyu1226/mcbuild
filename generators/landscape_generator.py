...
class LandscapeGenerator:
    ...
    def generate(self, description: str) -> dict:
        ...
        根据描述生成自然景观。
        Args:
            description: 景观的自然语言描述，例如 "一座小山和一条河流"。
        Returns:
            一个字典，包含景观的方块布局和结构信息。
        """
        print(f"LandscapeGenerator: Generating landscape for description: {description}")

        system_prompt = """你是一个Minecraft景观设计师。你的任务是根据用户的描述，设计一个Minecraft自然景观，并输出一个JSON数组，其中每个元素都是一个几何图元生成任务。每个任务都必须包含'tool'和'args'字段。

可用的几何图元工具及其参数示例：
- 'pyramid': {'x': 0, 'y': 0, 'z': 0, 'base_size': 10, 'block_type': 'minecraft:stone'} (用于山脉)
- 'line': {'x1': 0, 'y1': 0, 'z1': 0, 'x2': 20, 'y2': 0, 'z2': 0, 'block_type': 'minecraft:water'} (用于河流)
- 'circle': {'x': 0, 'y': 0, 'z': 0, 'radius': 10, 'block_type': 'minecraft:water'} (用于湖泊)
- 'sphere': {'x': 0, 'y': 0, 'z': 0, 'radius': 5, 'block_type': 'minecraft:dirt'} (用于小丘)

重要提示:
1.  所有坐标都是相对坐标，以(0,0,0)为基准。
2.  'block_type'必须是有效的Minecraft方块ID，例如'minecraft:stone'、'minecraft:water'、'minecraft:dirt'。
3.  请严格按照JSON数组格式返回，不要包含任何额外说明或代码块标记。"""    
        user_prompt = f"设计一个Minecraft自然景观：{description}"
        print(f"LandscapeGenerator: Sending prompt to LLM: {user_prompt}")

        success, llm_output = get_llm_response(self.llm_client, system_prompt, user_prompt)

        if not success:
            print(f"LandscapeGenerator: Error from LLM: {llm_output}")
            return {"description": description, "generated_structure": {}, "blocks": []}
        
        generated_components = llm_output
        print(f"LandscapeGenerator: Received LLM output: {generated_components}")
        ...
        print(f"LandscapeGenerator: Finished generating plan for: {description}")
        return build_plan

if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='Landscape Generator for Minecraft AI Builder.')
    ...
    generator = LandscapeGenerator(llm_client)
    ...
    print(f"LandscapeGenerator: Saved build plan to {output_filepath}")
