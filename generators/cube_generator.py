...
class CubeGenerator:
    """
    立方体生成器 - 负责生成各种尺寸和类型的立方体结构，可实心或空心。
    """
    ...
    def generate(self, description: str) -> dict:
        ...
        根据描述生成立方体结构。
        Args:
            description: 立方体的自然语言描述，例如 "一个5x5x5的空心玻璃立方体"。
        Returns:
            一个字典，包含立方体的方块布局和结构信息。
        """
        print(f"CubeGenerator: Generating cube for description: {description}")

        system_prompt = """你是一个Minecraft立方体设计师。你的任务是根据用户的描述，设计一个Minecraft立方体结构，并输出一个JSON数组，其中每个元素都是一个几何图元生成任务。每个任务都必须包含'tool'和'args'字段。

可用的几何图元工具及其参数示例：
- 'cube': {'x': 0, 'y': 0, 'z': 0, 'size_x': 5, 'size_y': 5, 'size_z': 5, 'block_type': 'minecraft:stone', 'hollow': false} (用于实心立方体)
- 'hollow_cube': {'x': 0, 'y': 0, 'z': 0, 'size_x': 5, 'size_y': 5, 'size_z': 5, 'block_type': 'minecraft:glass'} (用于空心立方体)

重要提示:
1.  所有坐标都是相对坐标，以(0,0,0)为基准。
2.  'block_type'必须是有效的Minecraft方块ID，例如'minecraft:stone'、'minecraft:glass'。
3.  **如果用户要求空心立方体，请务必使用'hollow_cube'工具。**
4.  请严格按照JSON数组格式返回，不要包含任何额外说明或代码块标记。"""    
        user_prompt = f"设计一个Minecraft立方体：{description}"
        print(f"CubeGenerator: Sending prompt to LLM: {user_prompt}")

        success, llm_output = get_llm_response(self.llm_client, system_prompt, user_prompt)

        if not success:
            print(f"CubeGenerator: Error from LLM: {llm_output}")
            return {"description": description, "generated_structure": {}, "blocks": []}
        
        generated_components = llm_output
        print(f"CubeGenerator: Received LLM output: {generated_components}")
        ...
        print(f"CubeGenerator: Finished generating plan for: {description}")
        return build_plan

if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='Cube Generator for Minecraft AI Builder.')
    ...
    generator = CubeGenerator(llm_client)
    ...
    print(f"CubeGenerator: Saved build plan to {output_filepath}")